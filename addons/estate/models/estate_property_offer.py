...
class EstatePropertyOffer(models.Model):
    ...
    def create(self, vals):
        ...
        if property_record.state == 'sold':
            raise UserError("You cannot create an offer for a property that is already sold.")

        if property_record.offer_ids:
            max_price = max(property_record.offer_ids.mapped('price'))
            if vals['price'] < max_price:
                raise UserError("The offer amount must be higher than the existing offers.")
